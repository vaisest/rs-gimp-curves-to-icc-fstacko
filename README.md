...
This repository contains a short Rust program that will convert GIMP preset files exported from the "Curves" tool to sRGB ICC profiles with an embedded gamma table. This is primarily useful as a way of adding custom calibration to the Windows desktop, which is not colour managed, but does support loading a gamma table onto the GPU. This is similar to how Nvidia control panel can change gamma, but the result should be much more customisable and less ugly.
